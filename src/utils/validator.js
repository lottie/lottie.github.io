  ...
  _get_all_props(prop_list) {
    if (!prop_list.resolved) {
      ...
      for (let ref of prop_list.references)
        for (let prop of this.get_all_props(ref)) prop_list.properties.add(prop)
    }
    ...
    for (let [name, sub_schema] of Object.entries(schema)) {
      if (name == "properties") {
        ...
        for (let i = 0; i < sub_schema.length; i++) {
          ...
          let oneof_list = id.endsWith("-property")
            ? prop_list
...
function kebab_to_title(kebab) {
  return kebab
    .split("-")
    .map(
      chunk => chunk.charAt(0).toUpperCase() + chunk.substring(1).toLowerCase()
    )
    .join(" ")
}

function custom_discriminator(
  propname,
  fail_unknown,
  ...
  function validate_fn(schema, data, parent_schema, data_cxt) {
    ...
    if (value === undefined) {
      if (default_value === undefined) return true
      value = default_value
    }
...
export class Validator {
  constructor(AjvClass, schema_json, docs_url = "") {
    ...
    for (let [cat, sub_schemas] of Object.entries(this.defs)) {
      ...
      for (let [obj, sub_schema] of Object.entries(sub_schemas)) {
        ...
        let id = `#/$defs/${cat}/${obj}`
        prop_map.extract_all_properties(
          sub_schema,
          id,
          prop_map.create(id, sub_schema),
          false
        )
      }
    }
    ...
    for (let [pname, pschema] of Object.entries(this.defs.properties)) {
      if (pname.endsWith("-property"))
        this._patch_property_schema(
          pschema,
          schema_id + "#/$defs/properties/" + pname
        )
    }
    ...
    this.validator = new AjvClass({
      ...
      keywords: [
        ...
        {
          keyword: "asset_oneof",
          validate: function validate_asset(
            schema,
            data,
            parent_schema,
            data_cxt
            ...
            if ("layers" in data)
              target_schema = this.getSchema(
                schema + "#/$defs/assets/precomposition"
              )
            else target_schema = this.getSchema(schema + "#/$defs/assets/image")
        ...
        {
          keyword: "keyframe",
          validate: function validate_keyframe(
            schema,
            data,
            parent_schema,
            data_cxt
        ...
        {
          keyword: "enum_oneof",
          validate: function validate_enum(
            schema,
            data,
            parent_schema,
            data_cxt
            ...
            for (let value of schema) if (value.const === data) return true
        ...
        },
        {
          ...
          validate: function validate_asset_reference(
            schema,
            data,
            parent_schema,
            data_ctx
        ...
        },
        {
          ...
          validate: function warn_extra_props(
            schema,
            data,
            parent_schema,
            data_cxt
  ...
  _patch_property_schema(schema, id) {
    if (id.endsWith("gradient-property")) {
      return this._patch_property_schema(
        schema.properties.k,
        id + "/properties/k"
      )
    }
    ...
    if (id.endsWith("splittable-position-property")) {
      ...
      schema.splitpos_oneof = {
        [true]: {
          id: this.schema["$id"] + "#/$defs/properties/split-position",
        },
        [false]: {
          id: this.schema["$id"] + "#/$defs/properties/position-property",
        },
      }
  ...
  validate(string) {
    ...
    if (!this._validate_internal(data))
      errors = this._validate_internal.errors.map(e =>
        this._cleaned_error(e, data)
      )
  ...
  _cleaned_error(error, data, prefix = "") {
    ...
    for (const path_part of path_parts) {
      if (path_part === "#" || path_part === "") continue
      ...
      if (data.ty) path_names.push(data.nm)
    }
